...
* LockJar.lock( jarfile = 'Jarfile', opts = {} ): From the jarfile, creates a lock file. Default jarfile lock is *Jarfile.lock*, can be changed by setting opts = { :lockfile => 'NewLockFile' }. The local repo defined in the Jarfile can be override by setting opts = { :local_repo => 'path/to/local/repo' }
...
* LockJar.list(*args): Lists all dependencies as notations for scopes from the local file. Defaults scopes are *compile* and *runtime*. Default lock file is *Jarfile.lock*.
...
* LockJar.load(*args): Loads all dependencies to the classpath. Defaults scopes are *compile* and *runtime*. Default lock file is *Jarfile.lock*.
  * args of a String will set the Jarfile, e.g. 'Jarfile.different'
  * args of an Array will set the scopes, e.g. ['compile','test']
  * args of a Hash will set the options, e.g. { :local_repo => 'path' }
    * :local_repo sets the local repo path
