...
* LockJar.lock( jarfile = 'Jarfile', opts = {} ): From the jarfile, creates a lock file. Default lock jar file Jarfile.lock, can be changed by setting opts = { :lockfile => 'NewLockFile' }. The local repo defined in the Jarfile can be override by setting opts = { :local_repo => 'path/to/local/repo' }
